# MediawikiAction SDK test runner
...
class MediawikiActionTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MediawikiActionTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MediawikiActionTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MediawikiActionTestRunner.getenv("MEDIAWIKI_ACTION_TEST_LIVE")
        override = MediawikiActionTestRunner.getenv("MEDIAWIKI_ACTION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MediawikiActionTestRunner.getenv(key)
        ...
        explain = MediawikiActionTestRunner.getenv("MEDIAWIKI_ACTION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MediawikiActionTestRunner._test_control is not None:
            return MediawikiActionTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MediawikiActionTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MediawikiActionTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MediawikiActionTestRunner.load_test_control()
...
def load_env_local():
    MediawikiActionTestRunner.load_env_local()


def env_override(m):
    return MediawikiActionTestRunner.env_override(m)


def entity_data(v):
    return MediawikiActionTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MediawikiActionTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MediawikiActionTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MediawikiActionTestRunner.load_test_control()


def live_client_options():
    return MediawikiActionTestRunner.live_client_options()


def live_delay_ms():
    return MediawikiActionTestRunner.live_delay_ms()
