...
|---|---|---|
| `ts/` | @voxgig-sdk/mediawiki-action-sdk | `.github/workflows/publish-ts.yml` |
...
`ts` target it runs:
    ...
    npm trust github @voxgig-sdk/mediawiki-action-sdk \
      --repository voxgig-sdk/mediawiki-action-sdk \
      --file publish-ts.yml \
      --allow-publish
...
`npm trust list @voxgig-sdk/mediawiki-action-sdk` shows the configuration, and
`npm trust revoke @voxgig-sdk/mediawiki-action-sdk --id=<id>` removes one.
