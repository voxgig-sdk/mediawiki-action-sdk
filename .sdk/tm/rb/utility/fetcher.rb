# MediawikiAction SDK utility: fetcher
...
module MediawikiActionUtilities
  ...
  DefaultHttpFetch = ->(fullurl, fetchdef) {
    ...
    begin
      ...
      request['User-Agent'] = 'Mozilla/5.0 (compatible; MediawikiActionSDK/1.0)' unless has_ua
      request.body = body_str if body_str.is_a?(String)

      http = MediawikiActionUtilities.http_checkout(key, uri, proxy)
      begin
        # Net::HTTP retries idempotent requests; an EOF can follow a committed POST.
        resp = http.request(request)
      rescue StandardError
        MediawikiActionUtilities.http_discard(http)
        raise
      end
      MediawikiActionUtilities.http_checkin(key, http)
