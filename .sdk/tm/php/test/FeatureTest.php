...
// MediawikiAction SDK feature test
//
...
class FtCtrl extends MediawikiActionControl
{
...
class FtClient
{
    ...
    private MediawikiActionUtility $utility;

    public function __construct(MediawikiActionUtility $utility, array $options)
    {
    ...
    public function get_utility(): MediawikiActionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MediawikiActionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MediawikiActionContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MediawikiActionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MediawikiActionConfig::make_feature($name);
    ...
    private static function buildUrl(MediawikiActionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MediawikiActionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MediawikiActionResponse($fetched);
        }
    ...
    private function fail(MediawikiActionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MediawikiActionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
