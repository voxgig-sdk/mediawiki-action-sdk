...
require_relative 'MediawikiAction_types'
...
class MediawikiActionSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MediawikiActionHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MediawikiActionHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MediawikiActionFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MediawikiActionUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MediawikiActionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MediawikiActionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MediawikiActionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MediawikiActionError.new(
        "#{op}_allow",
        "MediawikiActionSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue MediawikiActionError => err
    ...
    if fetched.is_a?(Hash)
      status = MediawikiActionHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MediawikiActionError.new(
        "graphql_error", "MediawikiActionSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MediawikiActionSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
