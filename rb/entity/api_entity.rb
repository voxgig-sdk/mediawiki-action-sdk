...
class ApiEntity
  ...
  def data_set(args)
    if args
      @_data = MediawikiActionHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MediawikiActionHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Api, Hash] the loaded Api; raises MediawikiActionError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MediawikiActionHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # Create a new Api.
  #
  # @param reqdata [ApiCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Api, Hash] the created Api; raises MediawikiActionError on failure
