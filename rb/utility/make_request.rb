...
module MediawikiActionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MediawikiActionResponse.new({})
    ...
    if fetch_err
      ...
      response = MediawikiActionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MediawikiActionResponse.new(fetched)
    else
