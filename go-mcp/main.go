...
func main() {
	...
	// Configure from the environment: MEDIAWIKI_ACTION_APIKEY carries the API key and
	// MEDIAWIKI_ACTION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MEDIAWIKI_ACTION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MEDIAWIKI_ACTION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMediawikiActionSDK(opts)
	server := mcp.NewServer(
		&mcp.Implementation{
			Name:    "mediawiki-action",
			Version: "0.0.0",
		},
		nil,
	)
