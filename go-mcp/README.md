# mediawiki-action-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MediawikiAction SDK as
two agent tools — `mediawiki-action_list` and `mediawiki-action_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mediawiki-action-mcp)
make build

# 2. Provide credentials via the environment
export MEDIAWIKI_ACTION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user mediawiki-action \
  -- /absolute/path/to/mediawiki-action-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./mediawiki-action-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// mediawiki-action_list: first page of records
{ "entity": "api" }
{ "entity": "api", "query": { } }

// mediawiki-action_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mediawiki-action-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mediawiki-action \
     -- "$PWD"/dist/*/mediawiki-action-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mediawiki-action_list` and `mediawiki-action_load` tools now appear
   in new sessions. Ask the agent to *"list api using mediawiki-action"*
   and it calls `mediawiki-action_list` with `{"entity":"api"}`.
...
```sh
export MEDIAWIKI_ACTION_APIKEY=sk_live_xxx            # API key
export MEDIAWIKI_ACTION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `mediawiki-action_list` tool
...
### Call the `mediawiki-action_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `mediawiki-action_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mediawiki-action_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MEDIAWIKI_ACTION_APIKEY` | API key sent with every request. |
| `MEDIAWIKI_ACTION_BASE` | Optional override of the API base URL. |
...
```sh
./mediawiki-action-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mediawiki-action_load","arguments":{"entity":"api","query":{"id":1}}}}'
```
