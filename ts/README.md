# MediawikiAction TypeScript SDK



The TypeScript SDK for the MediawikiAction API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Api()` — each with a small set of operations (`load`, `create`)
...
```ts
const client = new MediawikiActionSDK({ apikey: '...' })
...
```ts
new MediawikiActionSDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `MediawikiActionSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `MediawikiActionSDK.test(testopts?, sdkopts?)` | `MediawikiActionSDK` | Create a test-mode client. |
...
| `client` | `client(): MediawikiActionSDK` | Return the parent SDK client. |
...
mediawiki-action/
├── src/
│   ├── MediawikiActionSDK.ts        # Main SDK class
