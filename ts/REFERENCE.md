# MediawikiAction TypeScript SDK Reference

Complete API reference for the MediawikiAction TypeScript SDK.
...
```ts
new MediawikiActionSDK(options?: object)
```
...
#### `MediawikiActionSDK.test(testopts?, sdkopts?)`
...
**Returns:** `MediawikiActionSDK` instance in test mode.
...
Alias for `MediawikiActionSDK.test()`.
...
```ts
const result = await client.Api().load({ action: 'action' })
```
...
Return the parent `MediawikiActionSDK` instance.
