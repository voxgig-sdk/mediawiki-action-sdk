...
//     (\`new MediawikiActionSDK(...)\` / \`MediawikiActionSDK.test(...)\` become
//     \`MediawikiActionSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MediawikiActionSDK'
...
// rewrite real client construction (\`new MediawikiActionSDK(...)\` and any
// \`MediawikiActionSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MediawikiActionSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MediawikiActionSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
