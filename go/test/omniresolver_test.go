...
import (
	...
	omni "github.com/voxgig-sdk/mediawiki-action-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MediawikiActionError); is {
				out := map[string]any{
					"name":    "MediawikiActionError",
...
func omniCtx(arg any, client *sdk.MediawikiActionSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MediawikiActionSDK); is {
			client = live
