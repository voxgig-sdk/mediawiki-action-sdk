# MediawikiAction Golang SDK Reference

Complete API reference for the MediawikiAction Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *MediawikiActionSDK`
...
#### `Api(data map[string]any) MediawikiActionEntity`
...
```go
result, err := client.Api(nil).Load(map[string]any{"action": "action"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
