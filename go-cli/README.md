# mediawiki-action-cli

boru-driven command-line client **and** interactive REPL for the MediawikiAction
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mediawiki-action-cli)
make build

# 2. See usage (words, entities, env vars)
./mediawiki-action-cli --help

# 3. Provide credentials once, via the environment
export MEDIAWIKI_ACTION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./mediawiki-action-cli load 1 api            # {id:1} shorthand
./mediawiki-action-cli load '{id:1}' api       # explicit match map

# 5. Override the API base URL for a single call
MEDIAWIKI_ACTION_BASE=https://api.example.com ./mediawiki-action-cli load 1 api

# 6. No arguments -> interactive REPL
./mediawiki-action-cli
mediawiki-action> load 1 api
mediawiki-action> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mediawiki-action-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/mediawiki-action-cli load 1 api
   ```
...
```sh
./mediawiki-action-cli load 1 api          # scalar shorthand for {id:1}
./mediawiki-action-cli load '{id:1}' api     # explicit match map
```
...
./mediawiki-action-cli load 1 api
```
...
Run with no arguments to open a REPL (prompt `mediawiki-action>`). Each line is
evaluated as its own boru expression:

```text
$ ./mediawiki-action-cli
mediawiki-action> load 1 api
mediawiki-action> /help
mediawiki-action> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mediawiki-action-cli`. |
...
(`./mediawiki-action-cli <expr>`) and interactively (the REPL), and expressions compose the
