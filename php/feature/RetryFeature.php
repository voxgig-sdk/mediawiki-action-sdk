...
// MediawikiAction SDK retry feature
...
class MediawikiActionRetryFeature extends MediawikiActionBaseFeature
{
    ...
    public function init(MediawikiActionContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MediawikiActionContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MediawikiActionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MediawikiActionContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
